...
for label, kind in enumerate(['Cover', 'JMiPOD', 'JUNIWARD', 'UERD']):
    for path in glob('/home/shareData/ALASKA2/Cover/*.jpg'):
        dataset.append({
            'kind': kind,
...
class Fitter:
    ...
    def __init__(self, model, device, config):
        ...
        # add apex
        optimizer = torch.optim.AdamW(self.model.parameters(), lr=config.lr, eps=1e-4)
    ...
    def load(self, path):
        checkpoint = torch.load(path,map_location={'cuda:1':'cuda:0'})
...
def run_training():
    ...
    #fitter.load(f'{fitter.base_dir}/best-checkpoint-020epoch.bin')
